...
- **On demand, or when a team turns it on.** Run it after a plan is drafted — for example, a plan produced by [`visual-plan`](../visual-plan/SKILL.md). By default that skill does *not* invoke this one. A team that wants every plan challenged sets `## Challenge` with `Run: yes` in its custom-instructions file; `visual-plan` then runs this skill after the first render, with the round cap, the challenger invocation file, and the stalemate rule taken from that heading. A calling skill or a human owns the round cap and stalemate handling; this skill owns only the review.
- **The challenger must be a different model/agent than the author.** Same-model, fresh-context is a last resort, not the goal — an author rarely finds the assumption it just made. Invoke a genuinely different model however your environment allows (a second CLI agent, an API call to another model family, a different local model). When the custom instructions name a `Challenger:` file, use the exact commands in it; that file owns the model choice, the retry rule, and the timeout. If no different challenger can be reached after a reasonable attempt, proceed with the plan but record `Simplification challenge: skipped — <reason>` in the plan and say so plainly. Never skip silently.
